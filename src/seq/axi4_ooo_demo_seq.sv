...
//               Fires exactly 4 SINGLE-BEAT reads with distinct IDs (0..3) to
//               fixed addresses, back-to-back (no wait between), so all four
//               are outstanding together. With a reorder-enabled slave the R
//               responses return out of order (RID order != ARID order), while
//               each burst stays non-interleaved.
...
class axi4_ooo_demo_seq extends axi4_base_sequence;
    ...
    virtual task body();
        axi4_transaction rds[4];

        `uvm_info(get_type_name(),
                  "Starting OOO demo sequence (4 single-beat reads, IDs 0-3)",
                  UVM_MEDIUM)

        // Issue all 4 reads back-to-back WITHOUT waiting, so they are
        ...
        for (int i = 0; i < 4; i++) begin
            ...
            if (!rds[i].randomize() with {
                dir   == AXI4_READ;
                addr  == 32'h0000_1000 + (i * 32'h100);  // 0x1000/1100/1200/1300
                id    == i[3:0];                          // distinct IDs 0..3
        ...
        `uvm_info(get_type_name(),
                  "OOO demo sequence complete (4/4 reads returned)", UVM_MEDIUM)
    endtask : body
